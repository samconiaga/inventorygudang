@extends('layouts.app')

@include('customer.create')
@include('customer.edit')

@section('content')
<div class="section-header">
  <h1>Data Departemen</h1>
  <div class="ml-auto">
    <a href="javascript:void(0)" class="btn btn-primary" id="button_tambah_customer">
      <i class="fa fa-plus"></i> + Departemen
    </a>
  </div>
</div>

<div class="row">
  <div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        <div class="table-responsive">
          <table id="table_customer" class="table table-bordered table-striped" style="width:100%">
            <thead>
              <tr>
                <th style="width:70px">No</th>
                <th>Nama Departemen</th>
                <th style="width:160px">Opsi</th>
              </tr>
            </thead>
            <tbody></tbody>
          </table>
        </div>
      </div>
    </div>
  </div>
</div>
@endsection

@push('scripts')
<script>
$(function () {

  $.ajaxSetup({
    headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }
  });

  // Datatable (AJAX)
  const table = $('#table_customer').DataTable({
    paging: true,
    autoWidth: false,
    processing: true,
    ajax: {
      url: "/customer/get-data",
      type: "GET",
      dataSrc: function (json) {
        return json.data || [];
      },
      error: function(xhr){
        console.error("GET DATA CUSTOMER ERROR:", xhr.status, xhr.responseText);
      }
    },
    columns: [
      { data: null, render: function (d, t, r, meta) { return meta.row + 1; } },
      { data: "customer", defaultContent: "-" },
      {
        data: null,
        orderable: false,
        searchable: false,
        render: function (row) {
          return `
            <a href="javascript:void(0)" data-id="${row.id}"
              class="btn btn-icon btn-warning btn-sm mb-1 btn-edit-customer">
              <i class="far fa-edit"></i>
            </a>
            <a href="javascript:void(0)" data-id="${row.id}"
              class="btn btn-icon btn-danger btn-sm mb-1 btn-hapus-customer">
              <i class="fas fa-trash"></i>
            </a>
          `;
        }
      }
    ]
  });

  function reloadTable(){
    table.ajax.reload(null, false);
  }

  // =======================
  // TAMBAH
  // =======================
  $(document).on('click', '#button_tambah_customer', function(){
    $('#alert-customer').addClass('d-none').removeClass('d-block').html('');
    $('#customer').val('');
    $('#modal_tambah_customer').modal('show');
  });

  $(document).on('click', '#store', function(e){
    e.preventDefault();

    $('#alert-customer').addClass('d-none').removeClass('d-block').html('');

    $.ajax({
      url: '/customer',
      type: 'POST',
      data: { customer: $('#customer').val() },
      success: function(res){
        Swal.fire({ icon:'success', title: res.message || 'Berhasil' });
        $('#modal_tambah_customer').modal('hide');
        reloadTable();
      },
      error: function(xhr){
        if(xhr.status === 422){
          const err = xhr.responseJSON || {};
          if(err.customer?.[0]){
            $('#alert-customer').removeClass('d-none').addClass('d-block').html(err.customer[0]);
          }
        } else {
          console.error("STORE ERROR:", xhr.status, xhr.responseText);
        }
      }
    });
  });

  // =======================
  // EDIT
  // =======================
  $(document).on('click', '.btn-edit-customer', function(){
    const id = $(this).data('id');
    $('#alert-edit_customer').addClass('d-none').removeClass('d-block').html('');

    $.get(`/customer/${id}/edit`, function(res){
      $('#customer_id').val(res.data.id);
      $('#edit_customer').val(res.data.customer);
      $('#modal_edit_customer').modal('show');
    }).fail(function(xhr){
      console.error("EDIT GET ERROR:", xhr.status, xhr.responseText);
    });
  });

  $(document).on('click', '#update', function(e){
    e.preventDefault();

    const id = $('#customer_id').val();
    $('#alert-edit_customer').addClass('d-none').removeClass('d-block').html('');

    $.ajax({
      url: `/customer/${id}`,
      type: 'POST',
      data: { customer: $('#edit_customer').val(), _method:'PUT' },
      success: function(res){
        Swal.fire({ icon:'success', title: res.message || 'Berhasil' });
        $('#modal_edit_customer').modal('hide');
        reloadTable();
      },
      error: function(xhr){
        if(xhr.status === 422){
          const err = xhr.responseJSON || {};
          if(err.customer?.[0]){
            $('#alert-edit_customer').removeClass('d-none').addClass('d-block').html(err.customer[0]);
          }
        } else {
          console.error("UPDATE ERROR:", xhr.status, xhr.responseText);
        }
      }
    });
  });

  // =======================
  // HAPUS
  // =======================
  $(document).on('click', '.btn-hapus-customer', function(){
    const id = $(this).data('id');

    Swal.fire({
      title: 'Apakah Kamu Yakin?',
      text: 'Ingin menghapus data ini!',
      icon: 'warning',
      showCancelButton: true,
      cancelButtonText: 'TIDAK',
      confirmButtonText: 'YA, HAPUS!'
    }).then((r)=>{
      if(!r.isConfirmed) return;

      $.ajax({
        url: `/customer/${id}`,
        type: 'POST',
        data: { _method:'DELETE' },
        success: function(res){
          Swal.fire({ icon:'success', title: res.message || 'Terhapus' });
          reloadTable();
        },
        error: function(xhr){
          console.error("DELETE ERROR:", xhr.status, xhr.responseText);
          Swal.fire({ icon:'error', title:'Error', text:'Gagal menghapus data.' });
        }
      });
    });
  });

});
</script>
@endpush
